---
title: "MCMC Diagnostics - Extension to results from 4/28"
...
ind <- which(abs(draws[,beta_col]) > 30)
ext_draws <- draws[ind, ]
...
get_aggreg_plots <- function(fit, param, trim = F, trim_amount) {
  ...
  ext_medians <- data.frame(med = apply(ext_draws[,ind], 2, get_median))
  title <- paste0("Extreme Posterior Medians of ",param)
  print(ggplot(ext_medians, aes(x = med)) + geom_histogram(bins = 60) + ggtitle(title))
}
