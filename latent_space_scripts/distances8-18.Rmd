...
n_it <- dims[1]
n_param <- dims[3]
chain3 <- post[1:n_it,3,1:n_param]
```

```{r, echo = F}
beta_move <- which(chain3[,"beta"] < 1.3)
ind <- beta_move[1]
#ind <- ind - 3
```
...
after <- sample((ind+1):nrow(chain3), 1)
slice1 <- chain3[before,]
slice2 <- chain3[after,]
...
```{r, echo = F}
slice1 <- chain3[1:(ind-1),]
slice2 <- chain3[ind:nrow(chain3),]
