...
med_w <- data.frame(par = rownames(fit_summ), med = fit_summ[,6]) %>%
  ...
  mutate(Sector = labs[as.numeric(class)],
         time = paste0("Time ",time))
med_z$y <- rep(0,nrow(med_z))
med_w$y <- rep(0,nrow(med_w))
ggplot() +  geom_point(data = med_z, aes(y = med, x = y, color = Sector), alpha = 0.5) + 
  geom_point(data = med_w, aes(y = med, x = y, fill = Sector), 
             shape = 25, size = 4, color = "black")  + 
  xlab("") + ylab("Latent space posterior median") + 
...
```{r,echo=F}
mean_z <- data.frame(par = rownames(fit_summ), mean = fit_summ[,1]) %>%
  filter(grepl("z\\[", par)) %>%
  cSplit("par",direction = "wide") %>%
  mutate(worker = stri_extract_first_regex(par_1,"[0-9]+")) %>%
  select(-c("par_1")) %>%
  ...
  mutate(Sector = labs[value],
         time = paste0("Time ",time)) %>%
  select(-value) 
mean_w <- data.frame(par = rownames(fit_summ), mean = fit_summ[,1]) %>%
  filter(grepl("^w\\[", par)) %>%
  cSplit("par",direction = "wide") %>%
  mutate(class = stri_extract_first_regex(par_1,"[0-9]+"),
         time = stri_extract_first_regex(par_2,"[0-9]+")) %>%
  ...
  mutate(Sector = labs[as.numeric(class)],
         time = paste0("Time ",time))
mean_z$y <- rep(0,nrow(mean_z))
mean_w$y <- rep(0,nrow(mean_w))
ggplot() +  geom_point(data = mean_z, aes(y = mean, x = y, color = Sector), alpha = 0.5) + 
  geom_point(data = mean_w, aes(y = mean, x = y, fill = Sector), 
             shape = 25, size = 4, color = "black")  + 
  xlab("") + ylab("Latent space posterior mean") + 
  scale_x_discrete(labels = NULL, breaks = NULL) + 
  ggtitle("Latent Space Positions of Workers and Sectors") + 
  facet_wrap(~time)
```
