...
```{r, echo = F}
beta <- fit_summ["beta",6]
```


```{r, echo = F}
#samp <- sample(1:N, size = 20, replace = FALSE)
...
for (i in 1:N) {
  for (j in 1:K) {
    ...
    res_pers[i,j] <- -beta*past + beta*(1 - past) - diff
  }
}
# normalize 
res_exp <- exp(res)
...
```{r, echo = F, eval = F}
# plot 20 random people with and without persistence parameters 
samp <- sample(1:N, size = 20, replace = FALSE)
plot_dat <- data.frame(prob = res_norm[samp,1], prob_pers = res_pers_norm[samp,1])
ggplot(plot_dat, aes(x = prob, y = prob_pers)) + geom_point() +
  xlim(c(0,.5)) + ylim(c(0,.5)) + xlab("Base Probability") + 
  ylab("Probability with Persistence Parameters")
```
...
bus <- job[,1] == 6
# plot all 
plot_dat <- data.frame(prob = res_norm[,1], prob_pers = res_pers_norm[,1], ag = ag)
ggplot(plot_dat, aes(x = prob, y = prob_pers, color = ag)) + geom_point() +
  xlim(c(0,1)) + ylim(c(0,1)) + xlab("Base Probability") + 
  ylab("Probability with Persistence Parameters") + ggtitle("Probabilities for Agriculture") +
  scale_color_manual(values = c("black","red")) + labs(color = "A = 1")
plot_dat <- data.frame(prob = res_norm[,7], prob_pers = res_pers_norm[,7], ss = ss)
ggplot(plot_dat, aes(x = prob, y = prob_pers, color = ss)) + geom_point() +
  xlim(c(0,1)) + ylim(c(0,1)) + xlab("Base Probability") + 
  ylab("Probability with Persistence Parameters") + ggtitle("Probabilities for Social Services")  +
  scale_color_manual(values = c("black","red")) + labs(color = "A = 1")
plot_dat <- data.frame(prob = res_norm[,6], prob_pers = res_pers_norm[,6],bus = bus)
ggplot(plot_dat, aes(x = prob, y = prob_pers, color = bus)) + geom_point() +
  xlim(c(0,1)) + ylim(c(0,1)) + xlab("Base Probability") + 
  ylab("Probability with Persistence Parameters") + ggtitle("Probabilities for Business")  +
  scale_color_manual(values = c("black","red")) + labs(color = "A = 1")
```
