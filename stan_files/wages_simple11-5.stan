// doesn't run - likelihood evaluates to 0 
// because of s|r ~ bernoulli(0) when r = 1
data {
	int<lower=0> N; 
	int<lower=0> T;
	int<lower=0> K;
...
model{
	...
	for (t in 2:T) {
		for (n in 1:N) {
			r[t-1,n] ~ bernoulli(inv_logit(dot_product(beta',to_vector(x[n,t])) + v[job[n,t-1]]));
			if (r[t-1,n]==0) {
				s[t-1,n] ~ bernoulli(inv_logit(dot_product(delta',to_vector(x[n,t])) + u[job[n,t-1]]));
			}
			else {
				s[t-1,n] ~ bernoulli(0);
