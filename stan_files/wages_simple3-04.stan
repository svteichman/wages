// copied from 2/25
// changing adjusted parameters to take away normalization 
data {
	int<lower=0> N; 
	int<lower=0> T;
	int<lower=0> K;
...
transformed data {
	...
	// creating multinomial data for staying vs. leaving 
	for (n in 1:N) { 
		for (t in 1:(T-1)) {
...
parameters{
	...
	real<lower=0> sigmasq_a1;
	real<lower=0> sigmasq_a2;
	vector[Xnum] alpha;
	//real<lower=0> sigmasq_dbz;
	real<lower=0> sigmasq_dbz1;
	real<lower=0> sigmasq_dbz2;
	vector[Xnum] delta;
	vector[Xnum] beta;
	vector[Xnum] zeta;
...
transformed parameters {
	...
	real<lower=0> sigma_dbz1;
	real<lower=0> sigma_dbz2;
	...
	//vector[Xnum] adj_zeta;
	vector[K] p_vec;
	vector[K] new_p_vec;
	real eta[N,T-1];
	real lambda[N,T-1];
	real kappa[N,T-1];
//	vector [Xnum] avg_adj_x;

	sigma_w0 = sqrt(sigmasq_w0);
	sigma_w = sqrt(sigmasq_w);
	sigma_bk = sqrt(sigmasq_bk);
	//sigma_a = sqrt(sigmasq_a);
	sigma_a1 = sqrt(sigmasq_a1);
	sigma_a2 = sqrt(sigmasq_a2);
	//sigma_dbz = sqrt(sigmasq_dbz);
	sigma_dbz1 = sqrt(sigmasq_dbz1);
	sigma_dbz2 = sqrt(sigmasq_dbz2);
	sigma_uvq = sqrt(sigmasq_uvq);
	sigma_k = sqrt(sigmasq_k);
//	avg_adj_x[1] = mean(adj_x[1]);
//	avg_adj_x[1] = 1;
//	avg_adj_x[2] = mean(adj_x[2]);
//	avg_adj_x[2] = 1;
//	adj_alpha = alpha ./ avg_adj_x;
//	adj_beta = beta ./ avg_adj_x;
//	adj_delta = delta ./ avg_adj_x;
//	adj_zeta = zeta ./ avg_adj_x;
...
// computing probabilities of connecting between each worker and firm 
	for (n in 1:N) {
		for (t in 1:T) {
			for (k in 1:K) {
				...
				//print(dot_product(adj_alpha',to_vector(x[n,t])));
				p[n,t,k] = inv_logit(beta_k[k] - distance(to_vector(z[n]),to_vector(w[k,t])) + dot_product(alpha',to_vector(x[n,t])));
			}
	...
	for (n in 1:N) {
		...
		//eta is probability to stay, lambda is probability to stay in class but switch firm, kappa is probability to switch classes
			//eta[n,t-1] = exp(dot_product(adj_beta',to_vector(x[n,t])) + v[job[n,t-1]]);
			eta[n,t-1] = exp(dot_product(beta',to_vector(x[n,t])) + v[job[n,t-1]]);
			//lambda[n,t-1] = exp(dot_product(adj_delta',to_vector(x[n,t])) + u[job[n,t-1]]);
			lambda[n,t-1] = exp(dot_product(delta',to_vector(x[n,t])) + u[job[n,t-1]]);
			//kappa[n,t-1] = exp(dot_product(adj_zeta',to_vector(x[n,t])) + q[job[n,t-1]]);
			kappa[n,t-1] = exp(dot_product(zeta',to_vector(x[n,t])) + q[job[n,t-1]]);
...
model{
	...
	sigmasq_a1 ~ scaled_inv_chi_square(1,5);
	sigmasq_a2 ~ scaled_inv_chi_square(1,5);
	// shared variance for intercept for eta, lambda, kappa probabilities 
	//sigmasq_dbz ~ scaled_inv_chi_square(1,13);
	sigmasq_dbz1 ~ scaled_inv_chi_square(1,13);
	sigmasq_dbz2 ~ scaled_inv_chi_square(1,13);
	...
	alpha[1] ~ normal(0.0,sigma_a1);
	alpha[2] ~ normal(0.0,sigma_a2);
	//delta ~ normal(0.00005,sigma_dbz);
	delta[1] ~ normal(0.00005,sigma_dbz1);
	delta[2] ~ normal(0.00005,sigma_dbz2);
	//beta ~ normal(0.0001,sigma_dbz);
	beta[1] ~ normal(0.0001,sigma_dbz1);
	beta[2] ~ normal(0.0001,sigma_dbz2);
	//zeta ~ normal(0.0, sigma_dbz);
	zeta[1] ~ normal(0.0, sigma_dbz1);
	zeta[2] ~ normal(0.0, sigma_dbz2);
	...
	for (t in 2:T) {
		for (n in 1:N) {
			...
			m[t-1,n] ~ categorical(m_prob/sum(m_prob));
