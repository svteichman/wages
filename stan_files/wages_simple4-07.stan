// copied from 2/25
// experimenting with normalizing the x's instead of parameters 
data {
	int<lower=0> N; 
	int<lower=0> T;
	int<lower=0> K;
...
// computing probabilities of connecting between each worker and firm 
	for (n in 1:N) {
		for (t in 1:T) {
			for (k in 1:K) {
				//print(inv_logit(beta_k[k] - distance(to_vector(z[n]),to_vector(w[k,t])) + dot_product(alpha',to_vector(x_norm[n,t]))));
				p[n,t,k] = inv_logit(beta_k[k] - distance(to_vector(z[n]),to_vector(w[k,t])) + dot_product(alpha',to_vector(x_norm[n,t]))) + machine_precision();
			}
	...
	for (n in 1:N) {
		...
		//eta is probability to stay, lambda is probability to stay in class but switch firm, kappa is probability to switch classes
			...
			lambda[n,t-1] = exp(dot_product(delta',to_vector(x_norm[n,t])) + u[job[n,t-1]]);
