// copied from 3-9-21
// draw log wage from a normal distribution centered on worker effect plus firm effect
// draw firm effect from a normal distribution centered on sector effect with variance estimated from data
// single variance term 
data {
  int<lower=0> N; 
  int<lower=0> T;
  int<lower=0> K;
  ...
  real<lower=0> sector_precisions[K];
}
transformed data {
  int sector[N,T];
  real ly[N,T];
...
//  real<lower=0> sigma_i[N,T];
  ...
  //for (n in 1:N) {
    //sigmasq_i[n,1] = 1/tau;
    //for (t in 2:T) {
      //sigmasq_i[n,t] = 1/tau + 1/tau_k[sector[n,t]]*(s[t-1,n]) + 1/tau_m*(1-r[t-1,n])*(1-s[t-1,n]);
...
model{
 ...
 // tau_m ~ gamma(1, 1);
  for (k in 1:K) {
    tau_k[k] ~ gamma(sector_precisions[k],1);
  }
  // latent positions 
  for (k in 1:(K-1)) {
  ...
  for (m in 1:(N+M)) {
    unique_f[m] ~ normal(nu[new_job[m]], tau_k[new_job[m]]);
  }
  ...
  for (t in 1:T) {
    for (n in 1:N) {
      // ly[n,t] ~ normal(mu[n] + nu[sector[n,t]],sigma_i[n,t]);
