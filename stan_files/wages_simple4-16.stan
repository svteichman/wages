// copied from 4-11
// same as 4-11, but replacing \beta_k's with a single intercept 
data {
	int<lower=0> N; 
	int<lower=0> T;
	int<lower=0> K;
...
// computing probabilities of connecting between each worker and firm 
	for (n in 1:N) {
		for (t in 1:T) {
			for (k in 1:K) {
				//p[n,t,k] = inv_logit(beta_k[k] - distance(to_vector(z[n]),to_vector(w[k,t])));
				p[n,t,k] = inv_logit(beta_0 - distance(to_vector(z[n]),to_vector(w[k,t])));
			}
	...
	for (n in 1:N) {
		...
		//eta is probability to stay, lambda is probability to stay in class but switch firm, kappa is probability to switch classes
			eta[n,t-1] = exp(dot_product(adj_beta',to_vector(x_norm[n,t])) + v[job[n,t-1]]);
			//eta[n,t-1] = exp(dot_product(beta',to_vector(x_norm[n,t])) + v[job[n,t-1]]);
			...
			kappa[n,t-1] = exp(dot_product(adj_zeta',to_vector(x_norm[n,t])) + q[job[n,t-1]]);
			//kappa[n,t-1] = exp(dot_product(zeta',to_vector(x_norm[n,t])) + q[job[n,t-1]]);
