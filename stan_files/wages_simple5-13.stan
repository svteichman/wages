// copied from 4-30
// changing hyperparameters to that of Frial et al. 
data {
  int<lower=0> N; 
  int<lower=0> T;
  int<lower=0> K;
...
transformed parameters {
  ...
  for (n in 1:N) {
    for (k in 1:K) {
      ...
      for (t in 2:T) {
        p[n,t,k] = a[n,t-1,k]*gamma + (1-a[n,t-1,k])*beta - distance(to_vector(z[n]),to_vector(w[k,t]));
      }
    }
  }
  for (n in 1:N) {
...
model{
  ...
  for (n in 1:N) {
    z[n] ~ normal(0.0,1/sqrt(0.05)); // replaced tau_z with 0.05 here 
  }
  beta ~ normal(0.0,1/sqrt(tau_b)); 
  gamma ~ normal(0.0,1/sqrt(tau_g));
  for (t in 1:T) {
    for (n in 1:N) {
