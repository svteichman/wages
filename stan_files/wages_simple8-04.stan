// copied from 7-31
// adding back in latent space with known tau values 
data {
  int<lower=0> N; 
  int<lower=0> T;
  int<lower=0> K;
...
model{
//  tau_w0 ~ gamma(.5, 1);
//  alpha ~ normal(0, 1); // intercept for probabities
  tau_b ~ gamma(.5, 1);
  tau_m ~ gamma(1, 1);
  // latent positions 
  for (k in 1:K) {
   // w[k,1] ~ normal(0.0,1/sqrt(tau_w0));
      w[k,1] ~ normal(0.0, 1/sqrt(0.25));
    for (t in 2:T) {
